...
/*   Updated: 2024/02/13 18:55:06 by baouragh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_cmds(t_fd fd, int argc, char **argv, char **env)
{
	...
	i = fd.check_sum;
	len = ft_strlen(argv[i]) + ft_strlen(argv[argc -1]);
	len = ft_strncmp(argv[i], argv[argc - 1], len);
	mod = fd.i_place;
	cmds = argc - 4;
	...
	while (cmds--)
	{
		...
		else if (access(cmd, X_OK) && *argv[i] != '\0' && len)
